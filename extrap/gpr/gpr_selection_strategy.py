...
def suggest_points_gpr_mode(experiment, 
    ...
    if len(selected_callpaths) > 1:
        ...
        for l in range(len(selected_callpaths)):
            callpath = selected_callpaths[l]
                ...
                if l == 0:
                    ...
                    if possible_points[i] in remaining_points_gpr:
                        for o in range(len(remaining_points_gpr[possible_points[i]])):
                            remaining_points_gpr[possible_points[i]][o] = remaining_points_gpr[possible_points[i]][o] + np.mean(values)
                    else:
                        remaining_points_gpr[possible_points[i]] = values
                    if possible_points[i] in predicted_runtimes:
                        for o in range(len(predicted_runtimes[possible_points[i]])):
                            predicted_runtimes[possible_points[i]][o] = predicted_runtimes[possible_points[i]][o] + np.mean(runtime_values)
                    else:
                        predicted_runtimes[possible_points[i]] = runtime_values
        ...
        while True:
            ...
            for key, value in remaining_points_gpr.items():
                ...
                current_cost = calculate_selected_point_cost(experiment_gpr, callpath, metric, calculate_cost_manual, number_processes)
    ...
    # sum all callpaths runtime
        ...
        for l in range(len(experiment.callpaths)):
            callpath = experiment.callpaths[l]
