...
class MeasurementPointsPlot(GraphDisplayWindow):
    ...
    def draw_figure(self):
        ...
        if isinstance(model_list[0], list):
            X, Y, Z_List, z_List, X2, Y2, Z_List2, z_List2 = self.calculate_z_models(maxX, maxY, model_list, max_z)
            ax_all.mouse_init()
            ...
            for i in range(len(Z_List2)):
                ax_all.plot_surface(X2, Y2, Z_List2[i], color=dict_callpath_color[selected_callpaths[i]],
                                    rstride=1, cstride=1, antialiased=False, alpha=0.1)
