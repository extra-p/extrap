...
class CubeFileReader2(FileReader, ABC):
    ...
    def read_experiment(self, path: Union[Path, str], progress_bar: ProgressBar = DUMMY_PROGRESS) -> Experiment:
        ...
        path = Path(path)
        if not path.is_dir():
            raise FileFormatError(f'Cube file path must point to a directory: {path}')
        cubex_files = list(path.glob('*/[!.]*.cubex'))
        if not cubex_files:
            raise FileFormatError(f'No cube files were found in: {path}')
        progress_bar.total += len(cubex_files) + 6
        ...
        for parameter_value, point_group in groupby(reordered_files, key=itemgetter(1)):
            ...
            for path, _ in point_group:
                ...
                with CubexParser(str(path)) as parsed:
                    callpaths = self.make_callpath_mapping(parsed.get_root_cnodes())
