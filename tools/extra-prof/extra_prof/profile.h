...
class Profile {
    std::vector<std::string> empty_gpu_metric_names;
    const char *EXTRA_PROF = "EXTRA PROF";
    const char *version_name = "1.0";
    CallTreeNode *_call_tree = nullptr;
    std::vector<std::string> *_gpu_metric_names = &empty_gpu_metric_names;

public:
    Profile(CallTreeNode &call_tree) : _call_tree(&call_tree) {}
    Profile(CallTreeNode &call_tree, std::vector<std::string> &gpu_metric_names)
