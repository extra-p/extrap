...
#
# Copyright (c) 2021-2024, Technical University of Darmstadt, Germany
#
...
class TestComputationFunction(unittest.TestCase):
    ...
    def test_comparison3(self):
        # f0 = "oo*sign(-102117400755677*(p + 1)**(3/4)*log2(p + 1)**2 - 1315385896519540*(p + 1)**(1/4)*log2(p + 1)**2 - 50751283012506900000000*(p + 1)**(1/4) - 126145043414337*(p + 1)**(2/3)*log2(p + 1)**2 - 347932845155117*sqrt(p + 1)*log2(p + 1)**2 - 3860865509091260*log2(p + 1)**2 - 8998814109917810*log2(p + 1) + 141451981542016000000000)"
        # r = ComputationFunction.from_string(f0, True).sympy_function
        ...
        # r3 = r2.subs(dummy_infinity, sympy.oo)

        f1 = ("-9.41e-16*x**(3/4)*y**1.0*log2(x)**2.0*log2(y)**1.0 - 5.07e-15*x**(1/4)*y**(7/4)"
              " + 1.98e-10*x**(1/4)*y**(3/4)*log2(y)**1.0 + 3.40e-13*x**(1/4)*y**1.0*log2(y)**1.0"
              " + 8.73e-13*x**(1/4)*y**1.0*log2(y)**2.0 + 2.16e-6*x**(2/3) - 2.14e-10*x**1.0*log2(x)**2.0"
              " + 1.41e-14*y**(7/4) + 8.51e-15*y**(5/4) + 5.37e-11*y**(3/4)*log2(y)**1.0"
              " + 1.34e-8*y**(1/4)*log2(x)**1.0 + 2.47e-8*y**(1/4) + 1.90e-19*y**(5/3) + 1.14e-13*y**(4/3)"
              " + 1.09e-17*y**(3/2)*log2(x)**1.0*log2(y)**2.0 + 1.03e-8*sqrt(y)"
              " - 3.93e-15*y**1.0*log2(x)**1.0*log2(y)**1.0 + 5.64e-12*y**1.0*log2(x)**1.0*log2(y)**2.0"
              " + 1.39e-10*y**1.0*log2(x)**1.0 - 2.00e-11*y**1.0*log2(x)**2.0*log2(y)**1.0"
              " + 1.90e-9*y**1.0*log2(y)**1.0 + 1.91e-12*y**1.0*log2(y)**2.0 + 7.03e-9*y**1.0"
              " + 2.69e-6*log2(x)**1.0*log2(y)**1.0 + 0.00*log2(x)**1.0 + 3.03e-11*log2(x)**2.0*log2(y)**1.0"
              " - 2.34e-5*log2(x)**2.0 + 2.48e-6*log2(y)**1.0 + 0.36")
        f2 = ("5.46e-12*x**(7/4)*log2(y)**2.0 + 1.02e-23*x**(3/4)*y**(7/4)*log2(x)**2.0"
              " + 1.01e-16*x**(3/4)*y**(3/4)*log2(x)**1.0*log2(y)**2.0 + 1.01e-12*x**(3/4)*y**(2/3)*log2(y)**1.0"
              " + 2.51e-9*x**(3/4)*log2(x)**2.0 + 1.31e-22*x**(1/4)*y**(7/4)*log2(x)**2.0"
              " + 1.50e-15*x**(1/4)*y**(3/4)*log2(x)**1.0*log2(y)**2.0 + 2.48e-13*x**(1/4)*y**(3/4)*log2(y)**1.0"
              " + 2.81e-13*x**(1/4)*y**(3/4)*log2(y)**2.0 + 8.15e-22*x**(1/4)*y**(3/2)*log2(y)**2.0"
              " + 1.38e-11*x**(1/4)*y**1.0 + 2.31e-5*x**(1/4)*log2(x)**1.0 + 2.53e-8*x**(1/4)*log2(x)**2.0"
              " + 3.14e-7*x**(1/4)*log2(y)**1.0 + 0.00*x**(1/4) + 1.96e-11*x**(7/3) + 5.25e-8*x**(5/3)"
              " + 5.67e-14*x**(4/3)*y**(1/3)*log2(y)**2.0 + 2.91e-9*x**(4/3)*log2(x)**1.0 + 7.14e-8*x**(4/3)"
              " + 1.26e-23*x**(2/3)*y**(7/4)*log2(x)**2.0 + 6.01e-10*x**(2/3)*y**(1/4)*log2(x)**1.0"
              " + 7.07e-11*x**(2/3)*y**(1/4)*log2(y)**1.0 + 3.09e-9*x**(2/3)*sqrt(y)*log2(y)**2.0"
              " + 7.12e-10*x**(2/3)*sqrt(y) + 1.09e-6*x**(2/3)*log2(x)**1.0 - 5.59e-11*x**(2/3)*log2(x)**2.0"
              " + 6.48e-9*x**(2/3)*log2(y)**1.0 + 4.73e-7*x**(2/3)"
              " + 2.47e-10*x**(1/3)*y**(1/4)*log2(x)**1.0*log2(y)**1.0 + 4.35e-9*x**(1/3)*y**(1/4)*log2(y)**1.0"
              " + 1.72e-8*x**(1/3)*y**(1/3)*log2(y)**1.0 + 3.03e-9*x**(1/3)*y**(1/3) + 3.20e-19*x**(1/3)*y**(3/2)"
              " + 0.00*x**(1/3)*log2(x)**1.0*log2(y)**1.0 + 4.34e-8*x**(1/3)*log2(x)**1.0"
              " + 8.87e-8*x**(1/3)*log2(x)**2.0 + 1.37e-6*x**(1/3)*log2(y)**1.0 + 5.24e-5*x**(1/3)"
              " + 2.08e-11*x**(5/2)*y**(1/4) + 1.73e-10*x**(5/2)*log2(x)**2.0 - 8.74e-10*x**(5/2)"
              " + 2.66e-8*x**(3/2)*log2(x)**1.0 + 3.47e-23*sqrt(x)*y**(7/4)*log2(x)**2.0"
              " + 5.68e-13*sqrt(x)*y**(1/4)*log2(x)**2.0*log2(y)**2.0 + 4.11e-12*sqrt(x)*y**(2/3)"
              " + 1.16e-9*sqrt(x)*log2(x)**1.0 + 5.04e-11*sqrt(x)*log2(x)**2.0*log2(y)**2.0"
              " + 1.54e-8*sqrt(x)*log2(x)**2.0 + 1.41e-5*sqrt(x) + 3.15e-15*x**1.0*y**(2/3)*log2(y)**2.0"
              " + 6.95e-9*x**1.0*log2(x)**1.0 + 1.96e-7*x**1.0 - 8.66e-13*x**2.0*y**1.0"
              " + 5.54e-17*x**3.0*sqrt(y)*log2(x)**1.0*log2(y)**1.0 + 1.46e-11*x**3.0*log2(x)**1.0"
              " + 3.24e-10*x**3.0 + 2.51e-11*y**(4/5) + 8.99e-22*y**(7/4)*log2(x)**1.0"
              " + 3.86e-22*y**(7/4)*log2(x)**2.0 + 6.20e-19*y**(7/4) + 1.02e-17*y**(5/4)*log2(x)**1.0"
              " + 1.56e-18*y**(5/4)*log2(x)**2.0 + 1.77e-17*y**(5/4) + 5.92e-11*y**(3/4)*log2(x)**1.0*log2(y)**2.0"
              " + 2.36e-17*y**(3/4)*log2(x)**2.0*log2(y)**2.0 + 1.04e-10*y**(3/4)*log2(y)**2.0"
              " + 5.00e-10*y**(1/4)*log2(x)**1.0 + 4.70e-9*y**(1/4)*log2(y)**1.0 + 5.45e-11*y**(1/4)*log2(y)**2.0"
              " + 4.54e-7*y**(1/4) + 5.42e-20*y**(5/3) + 2.07e-15*y**(4/3)*log2(y)**1.0"
              " + 5.29e-10*y**(2/3)*log2(x)**1.0*log2(y)**1.0 + 7.42e-10*y**(2/3)*log2(y)**1.0"
              " + 4.85e-14*y**(2/3)*log2(y)**2.0 + 4.39e-12*y**(1/3)*log2(x)**1.0*log2(y)**2.0"
              " + 3.19e-11*y**(1/3)*log2(y)**2.0 - 4.40e-10*y**(1/3) + 1.40e-18*y**(3/2)*log2(x)**1.0"
              " + 2.18e-23*y**(3/2)*log2(x)**2.0*log2(y)**2.0 + 2.15e-20*y**(3/2)*log2(x)**2.0"
              " + 4.41e-22*y**(3/2)*log2(y)**2.0 + 9.70e-17*y**(3/2) + 2.43e-11*sqrt(y)*log2(x)**1.0"
              " + 7.10e-12*sqrt(y)*log2(y)**1.0 - 1.73e-9*sqrt(y)*log2(y)**2.0 + 3.31e-9*sqrt(y)"
              " + 1.02e-15*y**1.0*log2(x)**1.0 + 2.20e-15*y**1.0*log2(y)**1.0 + 7.03e-16*y**1.0*log2(y)**2.0"
              " + 1.66e-7*y**1.0 + 2.01e-6*log2(x)**1.0*log2(y)**1.0 + 5.34e-9*log2(x)**1.0*log2(y)**2.0"
              " - 6.65e-5*log2(x)**1.0 + 7.48e-9*log2(x)**2.0*log2(y)**1.0 + 7.84e-7*log2(x)**2.0"
              " + 8.72e-5*log2(y)**1.0 + 2.41e-7*log2(y)**2.0 - 0.02")

        cf1 = ComputationFunction.from_string(f1, True)
        cf2 = ComputationFunction.from_string(f2, True)
