...
namespace EXTRAP
{
SingleParameterHypothesis::SingleParameterHypothesis( void ) : m_function( NULL ), m_function_refcount( NULL )
{
}

SingleParameterHypothesis::~SingleParameterHypothesis()
{
}

SingleParameterHypothesis::SingleParameterHypothesis( SingleParameterFunction* f )
{
    this->m_function                 = f;
...
SingleParameterHypothesis::getFunction( void ) const
{
    return this->m_function;
}

double
SingleParameterHypothesis::getRSS( void ) const
{
    return this->m_RSS;
}

void
SingleParameterHypothesis::setRSS( double rss )
{
    this->m_RSS = rss;
}

double
SingleParameterHypothesis::getrRSS( void ) const
{
    return this->m_rRSS;
}

void
SingleParameterHypothesis::setrRSS( double rrss )
{
    this->m_rRSS = rrss;
}
...
double
SingleParameterHypothesis::getAR2( void ) const
{
    return this->m_AR2;
}

void
SingleParameterHypothesis::setAR2( double ar2 )
{
    this->m_AR2 = ar2;
}

double
SingleParameterHypothesis::getSMAPE( void ) const
{
    return this->m_SMAPE;
}

void
SingleParameterHypothesis::setSMAPE( double smape )
{
    this->m_SMAPE = smape;
}

double
SingleParameterHypothesis::getRE( void ) const
{
    return this->m_RE;
}

void
SingleParameterHypothesis::setRE( double re )
{
    this->m_RE = re;
}

std::vector<double>
SingleParameterHypothesis::getPredictedPoints( void )
{
    return this->m_predicted_points;
}

std::vector<double>
SingleParameterHypothesis::getActualPoints( void )
{
    return this->m_actual_points;
}

std::vector<double>
SingleParameterHypothesis::getPs( void )
{
    return this->m_ps;
}

std::vector<double>
SingleParameterHypothesis::getSizes( void )
{
    return this->m_sizes;
}

void
SingleParameterHypothesis::computeCost( const std::vector<DataPoint>& points )
{
    this->m_RSS  = 0;
    this->m_rRSS = 0;
    double smape = 0;
...
void
SingleParameterHypothesis::computeAdjustedRSquared( double TSS, const std::vector<DataPoint>& points )
{
    this->m_AR2 = 0.0;
    double adjR = 1.0 - ( this->m_RSS / TSS );

    double degrees_freedom = points.size() - this->m_function->getCompoundTerms().size() - 1;
...
void
SingleParameterHypothesis::computeCostLeaveOneOutCrossValidation( const std::vector<DataPoint>& points, DataPoint missing )
{
...
void
SingleParameterHypothesis::estimateParameters( const std::vector<DataPoint>& points )
{
    assert( m_function != NULL );

    int                 hypothesisTotalTerms = this->m_function->getCompoundTerms().size() + 1;
    ...
    for ( int i = 0; i < hypothesisTotalTerms; i++ )
    {
        ...
        for ( int rhs_index = 0; rhs_index < experimentCount; rhs_index++ )
        {
            ...
            if ( i > 0 )
            {
                CompoundTerm& compoundTerm = ( this->m_function->getCompoundTerms() )[ i - 1 ];
                compoundTerm.setCoefficient( 1 ); // reset coefficient
                eval = compoundTerm.evaluate( points[ rhs_index ].getParameterList() );
            }
            else
            {
                eval = 1;
    ...
    for ( int i = 0; i < hypothesisTotalTerms; i++ )
    {
        ...
        {
            ( this->m_function->getCompoundTerms() )[ i - 1 ].setCoefficient( 0 );
        }
        ...
        for ( int j = 0; j < hypothesisTotalTerms; j++ )
        {
            ...
            {
                ( this->m_function->getCompoundTerms() )[ i - 1 ].setCoefficient( ( this->m_function->getCompoundTerms() )[ i - 1 ].getCoefficient() + ( M[ irow + j ] * f[ j ] ) );
            }
        }
        if ( i == 0 )
        {
        ...
        {
            ( this->m_function->getCompoundTerms() )[ i - 1 ].setCoefficient( ( this->m_function->getCompoundTerms() )[ i - 1 ].getCoefficient() * regularization[ i ] );
        }
    }
...
bool
SingleParameterHypothesis::isValid( void )
{
    double cost = m_RSS;
...
double
SingleParameterHypothesis::calculateMaximalTermContribution( int termIndex, const std::vector<DataPoint>& points ) const
{
    std::vector<EXTRAP::CompoundTerm> ct = m_function->getCompoundTerms();
    assert( termIndex < ct.size() );
    double max = 0;
    for ( int i = 0; i < points.size(); i++ )
    {
        double contribution = fabs( ct[ termIndex ].evaluate( points[ i ].getParameterList() ) / points[ i ].getValue() );
        if ( contribution > max )
        {
            max = contribution;
        }
...
void
SingleParameterHypothesis::cleanConstantCoefficient( double epsilon, const std::vector<DataPoint>& points )
{
...
void
SingleParameterHypothesis::keepFunctionAlive( void )
{
...
void
SingleParameterHypothesis::freeFunction( void )
{
    assert( this->m_function_refcount != NULL );
    assert( this->m_function_refcount->func == this->m_function );
    ( this->m_function_refcount->count )--;
